% Descsion tree (DT)
function [AcR1Sum,AcR2Sum,AcR3Sum,AcR4Sum] = MyDT(n_test,XNorm,Label,LabelName)
...
for j = 1:n_test %generate random data
    ...
    % decision tree
    DT = fitctree(XTrain, LabelTrain);
%     view(DT, 'Mode','graph'); %Vuew the DT
    LabelTestDT = predict(DT, XTest);
    % calculate CM
    for i = 1:4
        for k = 1:20
            switch LabelTestDT((i-1)*20+k)
                case 1
                    CM(1,i) = CM(1,i)+1;
                case 2
                    CM(2,i) = CM(2,i)+1;
